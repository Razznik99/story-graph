...
/**
 * Recursively extracts text from a TipTap JSON structure.
 * @param json - The TipTap JSON node
...
/**
 * Generates a preview string from note content (which can be JSON or string).
 * @param content - The note content
 * @param maxLength - Maximum length of the preview
 * @returns The preview text
 */
export function getNotePreview(content: any, maxLength: number = 100): string {
  if (!content) return 'No content';

  let text = '';
  if (typeof content === 'string') {
    // Fallback for legacy HTML/Markdown strings
    text = content.replace(/<[^>]*>?/gm, '');
  } else if (typeof content === 'object') {
    try {
      text = extractTextFromTipTap(content);
    } catch (e) {
      console.error('Failed to parse TipTap JSON for preview', e);
